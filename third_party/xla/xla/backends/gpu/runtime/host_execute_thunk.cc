...
absl::Status HostExecuteAsyncEvents::AddEvent(se::StreamExecutor* executor,
                                              RunId run_id,
                                              HostExecuteEvent event) {
...
absl::Status HostExecuteAsyncEvents::HostExecuteEvent::WaitForEvent(
    se::Stream* stream) {
  tsl::BlockUntilReady(execute_event_);
  if (execute_event_.IsError()) {
    return execute_event_.GetError();
  }
  TF_RETURN_IF_ERROR(stream->WaitFor(stream_event_.get()));
  auto stream_event_status = stream_event_->PollForStatus();
  if (stream_event_status != se::Event::Status::kComplete) {
    return Internal("Stream event is not complete. It is in state %d.",
                    stream_event_status);
  }
  return absl::OkStatus();
}
...
absl::Status HostExecuteStartThunk::ExecuteOnStream(
  ...
  auto async_value_event = tsl::MakeConstructedAsyncValueRef<
      HostOffloadingExecutable::ExecuteEvent>();
  TF_ASSIGN_OR_RETURN(
      se::Stream * stream,
      GetStreamForExecution(Thunk::execution_stream_id(), params));

  TF_ASSIGN_OR_RETURN(auto stream_event, stream->parent()->CreateEvent());
  auto stream_event_ptr = stream_event.get();

  TF_RETURN_IF_ERROR(
      async_events_->AddEvent(stream->parent(), RunId(params.execution_id),
                              {std::move(stream_event), async_value_event}));
  ...
  // NOTE: We are making a shared pointer here because `execute` needs to be
  // copyable so that it can be scheduled on the thread pool.
  auto call_frame_ptr =
      std::make_shared<HostExecuteCallFrame>(std::move(call_frame));

  auto execute = [this, call_frame = std::move(call_frame_ptr), params,
                  stream_event_ptr, stream,
                  async_value_event = std::move(async_value_event)]() mutable {
    ...
    execute_event = execute_event.Map(
        [call_frame = std::move(call_frame), stream_event_ptr,
         stream](const HostOffloadingExecutable::ExecuteEvent& event) mutable {
          CHECK_OK(std::move(*call_frame).PublishResult())
              << "Failed to publish result.";
          CHECK_OK(stream->RecordEvent(stream_event_ptr))
              << "Failed to record stream event.";
          return event;
        });

    // Notify the shared event that the execution is done.
    execute_event.AndThen([async_value_event = std::move(async_value_event)](
                              absl::Status status) {
      if (!status.ok()) {
        async_value_event.SetError(status);
      } else {
        async_value_event.SetStateConcrete();
      }
    });
  };

  TF_RETURN_IF_ERROR(
      stream->DoHostCallbackWithStatus([execute = std::move(execute)] {
...
absl::Status HostExecuteDoneThunk::ExecuteOnStream(
  ...
  return event.WaitForEvent(stream);
}
