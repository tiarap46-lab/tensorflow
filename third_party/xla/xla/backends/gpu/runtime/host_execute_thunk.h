...
class HostExecuteAsyncEvents {
 public:
  class HostExecuteEvent {
   public:
    HostExecuteEvent(std::unique_ptr<se::Event> stream_event,
                     tsl::AsyncValueRef<HostOffloadingExecutable::ExecuteEvent>
                         execute_event)
        : stream_event_(std::move(stream_event)),
          execute_event_(std::move(execute_event)) {}

    absl::Status WaitForEvent(se::Stream* stream);

   private:
    std::unique_ptr<se::Event> stream_event_;
    tsl::AsyncValueRef<HostOffloadingExecutable::ExecuteEvent> execute_event_;
  };

  absl::Status AddEvent(se::StreamExecutor* executor, RunId run_id,
