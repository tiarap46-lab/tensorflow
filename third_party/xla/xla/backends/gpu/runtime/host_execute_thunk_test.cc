...
TEST(HostExecuteStartThunkTest, SingleArgSingleResult) {
  ...
  TF_ASSERT_OK(execute_event.WaitForEvent(stream.get()));
...
TEST(HostExecuteStartThunkTest, TestErrorPropagationFromExecuteEvent) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto execute_event,
                          thunk.async_events()->ExtractEvent(
                              stream_executor, RunId(params.execution_id)));
  EXPECT_FALSE(execute_event.WaitForEvent(stream.get()).ok());
}
...
TEST(HostExecuteDoneThunkTest, WaitingOnAvailableEvent) {
      ...
      run_options, allocations, stream.get(), stream.get(), nullptr, nullptr);
  {
    ...
    TF_ASSERT_OK_AND_ASSIGN(auto stream_event, stream->parent()->CreateEvent());
    TF_ASSERT_OK(stream->RecordEvent(stream_event.get()));

    TF_ASSERT_OK(async_events->AddEvent(
        stream_executor, RunId(params.execution_id),
        {std::move(stream_event), std::move(available_event)}));
  }
...
TEST(HostExecuteDoneThunkTest, WaitingOnErrorEvent) {
      ...
      run_options, allocations, stream.get(), stream.get(), nullptr, nullptr);
  {
    ...
    TF_ASSERT_OK(async_events->AddEvent(
        stream_executor, RunId(params.execution_id),
        {std::move(stream_event), std::move(error_event)}));
  }
